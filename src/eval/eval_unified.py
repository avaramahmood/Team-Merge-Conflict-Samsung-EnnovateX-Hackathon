"""
EVALUATION NOTEBOOK (unified, v5)
==================================
One notebook, three modes, so a base model and a think/answer SFT model are each
evaluated in their OWN native format. Same benchmark items, same gold labels,
same greedy decoding across modes; only the elicitation format and the answer
extractor change. That is the standard fair comparison (cf. lm-eval-harness and
the Open LLM Leaderboard: base models few-shot raw, instruct/reasoning models
zero-shot with their chat template). Identical format for both is NOT fairer, it
just penalizes whichever model the format is foreign to.

MODES
-----
  base_fewshot   : raw completion + few-shot exemplars + base extractors.
                   == eval v3 exactly. Run this on the BASE model.
  think_zeroshot : canonical <|im_start|> chat, zero-shot, <answer> parsing.
                   == eval v4 (budgets raised). Run this on the SFT checkpoints.
                   Use this one to rank uniform / centered / paper.
  think_fewshot  : canonical chat + few-shot exemplars rewritten in <think>/<answer>
                   form. Diagnostic: tests whether in-context examples help the
                   SFT model (it was trained single-turn, so this is mildly OOD).

HOW TO COMPARE
--------------
  base        -> base_fewshot   (its best, native)
  uniform     -> think_zeroshot
  centered    -> think_zeroshot
  paper       -> think_zeroshot
The base-vs-trained delta is "best base vs best trained", each native. The
checkpoint ranking (uniform/centered/paper) is fair because they share one mode.

  GSM8K      - Cobbe et al. 2021 ; MMLU - Hendrycks et al. 2021 ;
  StrategyQA - Geva et al. 2021 ; few-shot Brown 2020 ; CoT Wei 2022 ;
  harness reference Gao et al. 2023 (zenodo 10256836).
"""
...
def generate_batch(prompts, max_new_tokens):
    ...
    inputs  = tokenizer([prompts[i] for i in order], return_tensors="pt",
                        truncation=True, max_length=INPUT_MAX, padding=True).to("cuda")
    with torch.no_grad():
