"""
═══════════════════════════════════════════════════════════════════════════════
GRPO v8-A - SQA via VersaPRM DENSE REWARD SHAPING  (math PROF untouched)
═══════════════════════════════════════════════════════════════════════════════
Targets vs fresh baseline: GSM8K +2pp, StrategyQA +3pp, MMLU hold (eval-only).
START FRESH from pear_sft_epoch1.

SHARED CHANGE (all 3 variants): SQA rollouts now use the SAME 6-shot
decomposition prompt as eval (ROLLOUT_SQA_DECOMP=True). RL therefore
optimizes the exact distribution that is measured, and the longer
multi-step reasoning gives the PRMs real steps to grade (v6's failure
was min-agg over 2-4 short steps). Math rollouts are unchanged.

WHAT IS DIFFERENT FROM grpo.py (v7)
  * MATH: byte-identical to v7 - PROF + Qwen-Math-PRM, mix 40%, KL 0.005,
    LR 8e-6, difficulty-aware advantage (r - group_mean, no std). DO NOT TOUCH.
  * SQA:  v7 used plain GRPO on raw correctness (all 8 rollouts). That reinforces
    lucky 50/50 guesses. Here VersaPRM is brought back, but used CORRECTLY:
      - aggregation is MEAN over steps, not min (v7 showed min over 2-4 short
        sqa steps is near-random);
      - it is NOT used to SELECT traces (PROF-on-binary = noise, per v7); it is
        used as a DENSE, zero-mean advantage shaping term:
            adv_i = (r_i - mean_r)  +  BETA_PRM * (q_i - mean_q)
        where q_i = mean VersaPRM score of trace i. The PRM term is centered
        within the group, so it adds NO bias to the difficulty-aware outcome
        advantage - it only RE-RANKS traces within a group, pushing genuinely
        well-reasoned correct traces above lucky-correct ones (and well-reasoned
        wrong traces above sloppy wrong ones).
      - If VersaPRM fails its load-time validation, BETA_PRM contribution is
        skipped and sqa degrades gracefully to exact v7 plain GRPO.

CHECKPOINT / RESUME (changed per request)
  * No eval-gated "best" checkpoint. ONE directory (grpo_best/) always holds the
    LATEST policy, saved every CKPT_EVERY iters + at session end. Optimizer state
    is saved/restored too, so resume across 12h sessions is continuous.
  * Eval is decoupled: a one-time baseline eval (for deltas) + a final eval only.
    Set EVAL_EVERY>0 to also log periodic informational evals (never gates ckpt).
  * RESUME_FROM = root containing grpo_best/, grpo_progress.json, optim_state.pt.

MEMORY (96 GB): policy + ref + AdamW8bit resident; Qwen PRM and VersaPRM each
swap in/out around scoring (transient). Peak well under 70 GB.
═══════════════════════════════════════════════════════════════════════════════
"""
...
LLAMA_PRM   = "inputs/models/versa-prm/Llama-PRM800K"
VERSA_LORA  = "inputs/models/versa-prm/VersaPRM"
...
OPTIM_PATH = f"{OUT}/optim_state.pt"
os.makedirs(LOG_DIR, exist_ok=True)

# ── 3. HYPERPARAMETERS ───────────────────────────────────────────────────────
ITERATIONS       = 90          # total across resumes; ~22min/iter => ~3 commits of 11.5h.
PROMPTS_PER_ITER = 64
G                = 8
MATH_M_KEEP      = 4           # PROF keep for math only
LR               = 8e-6        # decisive movement (v6's 1.5e-6 was homeopathic)
WARMUP_ITERS     = 2
KL_COEF_MATH     = 0.005       # math PINNED to v7 value (do NOT disturb math)
KL_COEF_SQA      = 0.005       # sqa anchor (tune sqa independently of math)
EPS_LOW, EPS_HIGH = 0.20, 0.28
GRAD_CLIP        = 0.5
MICRO_BSZ        = 4           # sequences per batched backward
BETA_PRM         = 0.5         # weight of the zero-mean VersaPRM shaping term (sqa)

# training mix: SQA is the target with the most headroom. MMLU is EVAL-ONLY.
MIX = {"sqa": 0.60, "math": 0.40}
ROLLOUT_SQA_DECOMP = True       # roll out SQA in eval's 6-shot decomposition
                                # format (closes train/eval gap; math untouched)
...
EVAL_EVERY   = 20          # informational only; NEVER gates ckpt. evals at 0,20,40,60 +
                           # early-stops if targets met early. (not every 5-6, as asked.)
CKPT_EVERY   = 10**9       # SAVE ONCE, at the clean session-end stop only (no mid-run saves).
                           # mid-run saves are avoided to keep the working tree small
                           # (grpo_best bf16 ~5.6GB + optimizer ~5.6GB). The single end-of-run
                           # save_ckpt() below still fires on BOTH the time-budget break and
                           # normal completion, so cross-session resume is preserved.
MAX_SESSION_SECONDS = 41500   # stop cleanly at the wall-clock budget, then save
                              # at 12h (43200s); a timed-out commit fails and saves nothing.
EVAL_N       = {"math": 400, "sqa": 400, "mmlu": 150}   # bigger N = trustworthier delta
TARGET_DELTA = {"math": 2.0, "sqa": 3.0, "mmlu": -1.0}
...
def build_prompt(domain: str, question: str) -> str:
    # MATH: byte-identical to the pool builder (zero-shot chat template).
    # SQA : when ROLLOUT_SQA_DECOMP, use the SAME 6-shot decomposition prompt
    #       as eval_build_sqa, so training and eval share one distribution and
    #       the reasoning is multi-step (gradeable by the PRMs). _think_fewshot
    #       and _SQA_TA are defined in section 7; build_prompt is only CALLED
    #       in the training loop, so late name binding is fine.
    if domain == "sqa" and ROLLOUT_SQA_DECOMP:
        return _think_fewshot("commonsense",
                              f"Question: {str(question).strip()}", _SQA_TA)
...
# ═════════════════════════════════════════════════════════════════════════════
# 8b. VERSA PRM (Llama-PRM800K + LoRA; merged at load; token-prob scoring)
#     Used to SCORE sqa rollouts. Process scores are aggregated with MEAN (v7
#     showed min-agg over 2-4 short sqa steps is near-random). VersaPRM is
#     swapped on/off GPU exactly like the Qwen PRM. If validate() fails it is
#     disabled and sqa falls back to plain GRPO.
# ═════════════════════════════════════════════════════════════════════════════
class VersaPRM:
    STEP_DELIM = " \n\n\n\n"

    def __init__(self, base_dir: str, lora_dir: str):
        logger.log(f"\nLoading VersaPRM: base={base_dir} + lora={lora_dir}")
        self.ok = False
        self.tok = AutoTokenizer.from_pretrained(base_dir)
        if self.tok.pad_token is None:
            self.tok.pad_token = self.tok.eos_token
        self.tok.padding_side = "left"; self.tok.truncation_side = "left"

        self.model = AutoModelForCausalLM.from_pretrained(
            base_dir, torch_dtype=torch.bfloat16, device_map="cpu",
            attn_implementation="sdpa")
        self._merge_lora(lora_dir)
        self.model.eval()
        for p in self.model.parameters():
            p.requires_grad_(False)

        delim_ids     = self.tok.encode(self.STEP_DELIM, add_special_tokens=False)
        self.step_id  = delim_ids[-1]
        self.plus_id  = self.tok.encode("+", add_special_tokens=False)[-1]
        self.minus_id = self.tok.encode("-", add_special_tokens=False)[-1]
        self.plus_first = True
        self.on = False
        logger.log(f"  step_id={self.step_id} plus_id={self.plus_id} "
                   f"minus_id={self.minus_id}")

    def _merge_lora(self, lora_dir: str):
        try:
            from peft import PeftModel
            self.model = PeftModel.from_pretrained(self.model, lora_dir)
            self.model = self.model.merge_and_unload()
            logger.log("  LoRA merged via peft.")
            return
        except Exception as e:
            logger.log(f"  peft unavailable/failed ({e}); manual LoRA merge ...")
        cfg   = json.load(open(os.path.join(lora_dir, "adapter_config.json")))
        scale = cfg["lora_alpha"] / cfg["r"]
        ad = {}
        for fn in os.listdir(lora_dir):
            if fn.endswith(".safetensors"):
                with safe_open(os.path.join(lora_dir, fn), framework="pt") as f:
                    for k in f.keys():
                        ad[k] = f.get_tensor(k)
        ...
        for k in list(ad.keys()):
            ...
            if base_key in sd and kb in ad:
                A = ad[k].to(torch.float32); B = ad[kb].to(torch.float32)
                sd[base_key] += (B @ A * scale).to(sd[base_key].dtype)
                merged += 1
        self.model.load_state_dict(sd)
        logger.log(f"  manual LoRA merge: {merged} modules patched.")

    def to_gpu(self):
        if not self.on:
            self.model.to(DEV); self.on = True

    def to_cpu(self):
        if self.on:
            self.model.to("cpu"); self.on = False
            torch.cuda.empty_cache()

    def _fmt(self, q, trace):
        steps = split_steps(trace)
        return q + " \n\n" + self.STEP_DELIM.join(steps) + self.STEP_DELIM

    @torch.no_grad()
    def score(self, questions, traces, batch_size: int = PRM_BATCH):
        out = [[0.5] for _ in traces]
        texts = [self._fmt(q, t) for q, t in zip(questions, traces)]
        cand = ([self.plus_id, self.minus_id] if self.plus_first
                else [self.minus_id, self.plus_id])
        pidx = 0 if self.plus_first else 1
        for bs in range(0, len(texts), batch_size):
            batch = texts[bs:bs + batch_size]
            try:
                enc    = self.tok(batch, return_tensors="pt", padding=True,
                                  truncation=True, max_length=3072).to(DEV)
                logits = self.model(**enc).logits[:, :, cand]
                probs  = logits.float().softmax(dim=-1)[..., pidx]
                marker = (enc["input_ids"] == self.step_id)
                for i in range(len(batch)):
                    pos = marker[i].nonzero(as_tuple=True)[0]
                    if len(pos):
                        out[bs + i] = probs[i, pos].cpu().tolist()
                del enc, logits, probs, marker
            except torch.cuda.OutOfMemoryError:
                torch.cuda.empty_cache()
                if batch_size > 1:
                    sub = self.score(questions[bs:bs + batch_size],
    ...
    def validate(self) -> bool:
        ...
        good = ("<think>The Pacific covers about 165 million sq km while the "
                "Atlantic covers about 106 million sq km. The Pacific is the "
                "largest ocean on Earth, so yes.</think><answer>Yes</answer>")
        bad  = ("<think>Oceans are all the same size because water levels "
                "equalize everywhere, so neither is larger.</think>"
                "<answer>No</answer>")
        self.to_gpu()
        for _ in range(2):
            g   = float(np.mean(self.score([q, q], [good, bad])[0]))
            b   = float(np.mean(self.score([q, q], [good, bad])[1]))
            gap = g - b
            logger.log(f"  VersaPRM validation (plus_first={self.plus_first}): "
                       f"good={g:.3f} bad={b:.3f} gap={gap:+.3f}")
            if gap > 0.05:
                self.ok = True
                self.to_cpu()
                return True
            self.plus_first = not self.plus_first
        self.ok = False
        self.to_cpu()
        logger.log("  VersaPRM failed validation in both orientations. DISABLED.")
        logger.log("  sqa falls back to plain GRPO (no PRM signal).")
        return False
...
versa = None
if os.path.isdir(LLAMA_PRM) and os.path.isdir(VERSA_LORA):
    versa = VersaPRM(LLAMA_PRM, VERSA_LORA)
    versa.validate()
else:
    logger.log("VersaPRM paths missing; sqa falls back to plain GRPO.")
versa_ok = bool(versa and versa.ok)

optimizer = bnb.optim.AdamW8bit(
    [p for p in policy.parameters() if p.requires_grad],
    lr=LR, weight_decay=0.0)
if RESUME_FROM:
    _op = os.path.join(RESUME_FROM, "optim_state.pt")
    if os.path.exists(_op):
        try:
            optimizer.load_state_dict(torch.load(_op, map_location=DEV))
            logger.log(f"[resume] optimizer state restored from {_op}")
        except Exception as e:
            logger.log(f"[resume] optimizer restore failed ({e}); fresh moments")
...
def save_ckpt():
    """Save the LATEST policy + tokenizer ONLY (no optimizer state).
    The 7B policy is ~15.2GB in bf16; the AdamW8bit optimizer state is ~15GB
    more, and the two are large, so we save once at session end
    quota. So we checkpoint weights only. On resume the optimizer restarts with
    fresh Adam moments (re-warms within a few steps via WARMUP_ITERS) - an
    acceptable trade vs. not being able to save the model at all."""
    policy.save_pretrained(CKPT_DIR, safe_serialization=True)
    policy_tok.save_pretrained(CKPT_DIR)
    logger.log(f"  ✓ latest checkpoint (weights only) -> {CKPT_DIR}")

# ═════════════════════════════════════════════════════════════════════════════
# 14. (EVAL REMOVED) - no baseline / periodic / final eval during training.
#     Eval wastes GPU time. Train + checkpoint only; run eval_unified_v5
#     separately afterwards on the saved checkpoint at outputs/grpo_best.
# ═════════════════════════════════════════════════════════════════════════════
...
logger.banner(
    ...
    f"prompts/iter={PROMPTS_PER_ITER} G={G} | sqa=PRM-shaping(β={BETA_PRM}) "
    f"math=PROF(m={MATH_M_KEEP}) | adv=r-groupmean (no std) | "
    f"eps=[{EPS_LOW},{EPS_HIGH}] KLm={KL_COEF_MATH} KLs={KL_COEF_SQA} LR={LR} clip={GRAD_CLIP} | "
    f"mix={MIX} sqa_roll={'decomp' if ROLLOUT_SQA_DECOMP else 'zeroshot'} | "
    f"retire 8/8 + 0/8x{RETIRE_ZERO_AFTER}")

t_start = time.time()

while state["iter"] < ITERATIONS:
    if (time.time() - t_start) >= MAX_SESSION_SECONDS:
        logger.log(f"\n[session] {MAX_SESSION_SECONDS}s wall-clock budget reached at "
                   f"iter {state['iter']}/{ITERATIONS}; stopping cleanly. "
                   f"Checkpoint + progress + logs are saved below; resume next session.")
        break
    it    = state["iter"] + 1
    t_it  = time.time()
    items = next_items()
    ...
    if math_jobs:
        ...
        qwen_prm.to_cpu()
    if versa_ok:
        sqa_jobs = [(rec, texts) for rec, texts, _ in survivors
                    if rec["domain"] == "sqa"]
        if sqa_jobs:
            versa.to_gpu()
            for rec, texts in sqa_jobs:
                prm_lists[id(rec)] = versa.score(
                    [rec["question"]] * len(texts), texts)
            versa.to_cpu()
    ...
    for rec, texts, corr in survivors:
        ...
        else:   # sqa: dense VersaPRM advantage shaping, keep ALL rollouts
            if versa_ok and id(rec) in prm_lists:
                pl = prm_lists[id(rec)]
                q  = np.array([float(np.mean(p)) if p else 0.5 for p in pl])
                qc = (q - q.mean())
                advs = [a + BETA_PRM * float(c) for a, c in zip(advs, qc)]
    ...
    for cs in range(0, len(buf), MICRO_BSZ):
        ...
        try:
            ...
            for b, cur in zip(chunk, cur_list):
                ...
                kl_coef = KL_COEF_MATH if b["domain"] == "math" else KL_COEF_SQA
                term = pg + kl_coef * kl
    ...
    # ── 15h. periodic checkpoint (LATEST, not best). NO eval during training. ─────
    if it % CKPT_EVERY == 0:
        save_ckpt()
        save_progress()
...
print("\n" + "=" * 70)
print("SESSION DONE.")
print(f"  last scores : {state.get('best_scores')}")
