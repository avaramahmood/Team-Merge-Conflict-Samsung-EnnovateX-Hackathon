"""
═══════════════════════════════════════════════════════════════════════════════
GRPO v8-B - SQA via VERIFIER-ONLY ANTI-GUESS REWARD  (no PRM; math PROF untouched)
═══════════════════════════════════════════════════════════════════════════════
...
WHAT IS DIFFERENT FROM grpo.py (v7)
  ...
  * SQA:  no extra model at all. The binary outcome is the same lucky-guess trap
    v7 leaves open. Here the reward is engineered to make GENUINE reasoning the
    only reliable way to score, using only the (free, exact) verifier signals:
        r = (+1 correct / -1 wrong)
            + 0.10 if well-formed <think>...</think><answer>...</answer>
            ± 0.10 think-conclusion (last Yes/No in <think>) agrees / disagrees
                   with the <answer>  (answer must FOLLOW from the reasoning)
            - 0.15 if correct but <think> has < 40 chars  (anti lucky guess)
        adv_i = r_i - mean_r   (difficulty-aware, no std - Dr.GRPO style)
    Outcome dominates; the shaping terms only break ties between traces that
    reach the same outcome, demoting "right by luck / wrong reasoning" traces.
  * No VersaPRM => fastest of the three => most iterations inside the 90h budget,
    and zero dependency on a flaky commonsense PRM.
...
# ── sqa anti-guess shaped reward (verifier-only; no PRM) ─────────────────────
MIN_THINK_CHARS_SQA = 40
FMT_BONUS_SQA       = 0.10
CONSIST_BONUS_SQA   = 0.10
NOREASON_PEN_SQA    = 0.15

def is_well_formed_sqa(t: str) -> bool:
    return (bool(re.search(r"<think>.*?</think>",   t, re.DOTALL | re.IGNORECASE)) and
            bool(re.search(r"<answer>.*?</answer>", t, re.DOTALL | re.IGNORECASE)))

def _think_conclusion_sqa(think: str) -> Optional[str]:
    m = re.findall(r"\b(yes|no)\b", think, re.IGNORECASE)
    return m[-1].lower() if m else None

def sqa_shaped_reward(trace: str, gold: str, correct: bool) -> float:
    base = 1.0 if correct else -1.0
    if is_well_formed_sqa(trace):
        base += FMT_BONUS_SQA
    think = extract_think(trace)
    concl = _think_conclusion_sqa(think)
    ans   = _answer_block(trace)
    if concl is not None and ans is not None:
        ans_yn = normalize_sqa(ans)
        if ans_yn in ("yes", "no"):
            base += CONSIST_BONUS_SQA if concl == ans_yn else -CONSIST_BONUS_SQA
    if correct and len(think) < MIN_THINK_CHARS_SQA:
        base -= NOREASON_PEN_SQA
    return base
...
logger.banner(
    ...
    f"prompts/iter={PROMPTS_PER_ITER} G={G} | sqa=antiguess-reward "
...
while state["iter"] < ITERATIONS:
    ...
    for rec, texts, corr in survivors:
        ...
        else:   # sqa: anti-guess shaped reward, difficulty-aware, keep ALL
            rw   = np.array([sqa_shaped_reward(t, rec["gold"], c)
                             for t, c in zip(texts, corr)], dtype=np.float64)
            advs = (rw - rw.mean()).tolist()
