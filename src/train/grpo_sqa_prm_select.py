"""
═══════════════════════════════════════════════════════════════════════════════
GRPO v8-C - SQA via VersaPRM SELECTION (gate lucky-correct)  (math PROF untouched)
═══════════════════════════════════════════════════════════════════════════════
...
WHAT IS DIFFERENT FROM grpo.py (v7)
  * MATH: byte-identical to v7 - PROF + Qwen-Math-PRM, mix 40%, KL 0.005,
    LR 8e-6, difficulty-aware advantage. DO NOT TOUCH.
  * SQA:  VersaPRM is used as a SELECTOR (the PROF spirit) but fixed against the
    v7 failure modes:
      - aggregation is MEAN over steps (not min - min over 2-4 short sqa steps
        is near-random);
      - selection is one-sided: among the CORRECT traces of a prompt, drop the
        bottom-half by mean VersaPRM (the likely lucky-guess / unreasoned ones);
        keep the well-reasoned correct traces + ALL wrong traces (kept for the
        negative-gradient contrast). With < 2 correct, keep everything.
      - advantage stays difficulty-aware over the FULL group (r - group_mean,
        no std), so dropping a trace removes its gradient without distorting the
        per-problem difficulty signal.
      - If VersaPRM fails validation, sqa degrades to exact v7 plain GRPO.
    Contrast with v8-A: there VersaPRM shapes a CONTINUOUS reward for every
    trace; here it makes a HARD keep/drop decision on the positive class only.

CHECKPOINT / RESUME: identical policy to v8-A (latest-only ckpt every CKPT_EVERY
+ optimizer state; eval decoupled, off by default + final eval). See that file.
═══════════════════════════════════════════════════════════════════════════════
"""
...
logger.banner(
    ...
    f"prompts/iter={PROMPTS_PER_ITER} G={G} | sqa=PRM-select "
...
while state["iter"] < ITERATIONS:
    ...
    for rec, texts, corr in survivors:
        ...
        else:   # sqa: VersaPRM-gated selection of CORRECT traces
            keep_idx = list(range(len(texts)))
            if versa_ok and id(rec) in prm_lists:
                pl      = prm_lists[id(rec)]
                cor_idx = [i for i, c in enumerate(corr) if c]
                if len(cor_idx) >= 2:
                    qcor = [float(np.mean(pl[i])) if pl[i] else 0.5
                            for i in cor_idx]
                    thr  = float(np.median(qcor))
                    drop = {i for i, qq in zip(cor_idx, qcor) if qq < thr}
                    keep_idx = [i for i in range(len(texts)) if i not in drop]
        for i in keep_idx:
            if texts[i].strip():
